...
def main() -> None:
    ...
    ax.plot(x_abrupt / 1e-4, e_abrupt / 1e5, label=f"abrupt, VR={vr_sample:g} V")
    ax.plot(x_graded / 1e-4, e_graded / 1e5, label=f"linearly graded, VR={vr_sample:g} V")
    ...
    fig, ax = plt.subplots(figsize=(7.2, 4.8))
    ax.semilogy(vr_values, [row[2] for row in rows], label="electron, abrupt")
    ax.semilogy(vr_values, [row[3] for row in rows], label="hole, abrupt")
    ax.semilogy(vr_values, [row[5] for row in rows], "--", label="electron, graded")
    ax.semilogy(vr_values, [row[6] for row in rows], "--", label="hole, graded")
    ax.set_xlabel(r"$V_R$ / V")
    ax.set_ylabel(r"ionization rate / cm$^{-1}$")
